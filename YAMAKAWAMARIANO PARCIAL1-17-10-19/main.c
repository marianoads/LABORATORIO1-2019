...
void ordenarPartidoPorReferi(ePartido vectorPartido[],int tam);
void mostrarTodosLosReferisQueTenganMasDe4Partidos(eReferi vectorReferi[],int tam);
...
int main()
{
    ...
    eJugadores harcodeoJugadores[TAM] = {{1,"Lucas","Pratto",{1},'f',{15,05,1999},1},{2,"Lucas","Borre",{1},'f',{12,10,1999},1},{3,"Carlos","Tevez",{2},'m',{30,10,1929},1}
                                        ,{4,"Nicolas","Blandi",{5},'m',{21,05,1999},1},{5,"Lisandro","Lopez",{3},'m',{21,03,1985},1},{6,"Ramon","Avila",{2},'m',{30,11,1999},1}
                                        ,{7,"Angel","Romero",{5},'m',{30,05,1999},1},{8,"Matias","Zaracho",{3},'m',{11,9,1995},1},{9,"Enrique","Salas",{4},'m',{11,9,1990},1},{10,"Santiago","Diaz",{4},'f',{11,9,1990},1}};
    eReferi harcodeoReferis[TAM] = {{1,"Horacio","Elizondo",'m',"horacioelizondo@hotmail.com",{29,04,1980},4,1},{2,"Nestor","Pitana",'m',"nestorpitana@hotmail.com",{30,01,1979},3,1},
                                    {3,"Javier","Castrilli",'m',"javiercastri@gmail.com",{19,01,1974},3,1},{4,"Diego","Abal",'m',"abaldiego@yahoo.com",{11,02,1988},3,1},{5,"Pablo","Lunati",'m',"lupatip@hotmail.com",{31,04,1970},4,1}};
    ...
    while(seguir == 's')
    {
        ...
        case 1:
            ...
            {
            case 1:
                altaEquipo(harcodeoEquipos,TAM);
                break;
            case 2:
        ...
        case 2:
            ...
            {
            case 1:
                altaJugadores(harcodeoJugadores,harcodeoEquipos,TAM);
                break;
            case 2:
                system("cls");
                printf("\n\n\nJugadores Ordenados por sexp\n");
                ordenarJugadoresPorSexo(harcodeoJugadores,TAM);
                mostrarJugadores(harcodeoJugadores,TAM,harcodeoEquipos);
                printf("\n\n\nJugadores Ordenados por nombre ascendente\n");
            ...
            break;
        case 3:
            printf("***Menu referi***\n\n1-Alta referi\n2-Listar referis con mas de 4 partidos dirigidos\n3-Salir\n\nElija opcion");
            scanf("%d",&opcion);
            switch(opcion)
            {
            case 1:
                altaReferi(harcodeoReferis,TAM);
                break;
            case 2:
                mostrarTodosLosReferisQueTenganMasDe4Partidos(harcodeoReferis,TAM);
                break;
            case 3:
                break;
            default:
        ...
        case 4:
            ...
            {
            case 1:
                altaPartido(harcodeoPartidos,harcodeoEquipos,harcodeoReferis,TAM);
                break;
            case 2:
                printf("\n\n\Partidos Ordenados por fecha ascendente\n");
                ordenarPartidosPorFecha(harcodeoPartidos,TAM);
                mostrarPartidos(harcodeoPartidos,harcodeoEquipos,TAM);
                printf("\n\n\Partidos Ordenados por Referi\n");
                ordenarPartidoPorReferi(harcodeoPartidos,TAM);
                mostrarPartidos(harcodeoPartidos,harcodeoEquipos,TAM);
                system("pause");
                break;
            case 3:
...
void ordenarJugadoresPorSexo(eJugadores vectorJugadores[],int tam)
{
    int i,j;
    eJugadores auxStruct;
    for(i=0; i<tam-1; i++)
    {
        if(vectorJugadores[i].ocupado==1)
        {
            for(j=i+1; j<tam; j++)
            {
                if(vectorJugadores[j].ocupado==1)
                {
                    if(vectorJugadores[i].sexoJugadores>vectorJugadores[j].sexoJugadores)
                    {
                        auxStruct=vectorJugadores[i];
                        vectorJugadores[i]=vectorJugadores[j];
                        vectorJugadores[j]=auxStruct;
...
void mostrarElEquipoMasJoven(eJugadores vectorJugador[],eEquipos vectorEquipo[],int tam)
{
    ...
    float vectorEquiposPromedio[tam];
    float menor;
    ponerEnCeroVectorPromedio(vectorEquiposPromedio,tam);
    ...
    for(i=0; i<tam; i++)
    {
        if(vectorEquipo[i].codigoEquipos==codEquipo && vectorEquipo[i].ocupado==1)
        {
            ...
            vectorEquiposPromedio[codEquipo]=promedio;
            //printf("\nXX:%.2f",vectorEquiposPromedio[codEquipo]);
            printf("Equipo:%s  PromedioEdad:%.2f",equipo,promedio);
            printf("   Cantidad de personas :%.0f\n\n",contadorJugadores);
    ...
    menor=vectorEquiposPromedio[1];
    for(i=0; i<tam; i++)
    {
        if(vectorEquiposPromedio[i] != 0)
        {
            if(vectorEquiposPromedio[i]<menor)
            {
                menor=vectorEquiposPromedio[i];
            }
        }

    }
    printf("\n\n**El equipo con menor promedio de edad es:%.2f\n\n", menor);

}

void ponerEnCeroVectorPromedio(float vec[],int tam)
{
    for(int i=0; i<tam; i++)
    {
        vec[i]=0;
...
void mostrarTodosLosReferisQueTenganMasDe4Partidos(eReferi vectorReferi[],int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        if(vectorReferi[i].ocupado==1 && vectorReferi[i].cantidadPartidosDirigidos >= 4)
        {
            printf("\n\n**El refeeri que ha dirigido mas de 4 partidos es : %s %s con %d partidos**\n\n",vectorReferi[i].apellidoReferi,vectorReferi[i].nombreReferi,vectorReferi[i].cantidadPartidosDirigidos);
        }
    ...
    for(int i=0;i<tam;i++){
        if(vectorPartido[i].ocupado==1){
            for(int j=i+1;j<tam;j++){
                if(vectorPartido[j].ocupado==1){
                    if(vectorPartido[i].structReferi.codigoReferi>vectorPartido[j].structReferi.codigoReferi){
                        auxStruct=vectorPartido[i];
                        vectorPartido[i]=vectorPartido[j];
                        vectorPartido[j]=auxStruct;
                    }
                }
            }
        }
